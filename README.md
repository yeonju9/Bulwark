...
웹 기반 방치형 RPG. Melvor Idle 스타일의 생활+전투 스킬 시뮬레이션을 목표로 한다.
...
- 스킬 3종: 벌목 / 채광 / 대장기술 (제련 + 장비 제작)
- 작업 슬롯 시스템: 1슬롯으로 시작, 총 레벨 30에서 2슬롯, 90에서 3슬롯 해금
  (같은 스킬의 작업은 동시에 하나만, 슬롯이 차면 가장 오래된 작업 교체)
- 오프라인 진행 정산 (상한 12시간) + 정산 모달
- localStorage 저장 (5초 자동 저장), 인벤토리 판매 → 골드
- 서버: 세이브 GET/PUT 골격 (인메모리, 클라이언트는 아직 미연동)

## 로드맵

1. ~~프로토타입~~ ← 지금 여기
2. 수직 슬라이스: 스킬 5~6종, 전투/던전, 계정 + 서버 저장 연동 (PostgreSQL)
3. 콘텐츠 확장 + 폴리시 (사운드, 애니메이션, AI 생성 아이콘으로 교체)
4. 소프트 런칭 → 시즌 리그 / 거래소 / 월드 보스
